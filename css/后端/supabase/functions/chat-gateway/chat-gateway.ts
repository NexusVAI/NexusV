...
const HEALTH_DERIVED_DISABLE_WINDOW_MS = 30 * 60_000
const HEALTH_DERIVED_DISABLE_FAILURES = 2
...
async function getDisabledLineSet(): Promise<Set<string>> {
  ...
  disabledLineRefreshing = (async () => {
    try {
      ...
      const since = new Date(Date.now() - HEALTH_DERIVED_DISABLE_WINDOW_MS).toISOString()
      const { data: healthRows, error: healthError } = await supabase
        ...
        .limit(500)
      if (!healthError) {
        const healthByModel = new Map<string, { failures: number; lastFail: string; lastOk: string }>()
        for (const row of (healthRows || []) as Array<{ model_id?: string; success?: boolean; created_at?: string }>) {
          const modelId = String(row.model_id || '').trim()
          if (!modelId || !SERVER_MODEL_REGISTRY[modelId]) continue
          const createdAt = String(row.created_at || '')
          if (!createdAt) continue
          const stat = healthByModel.get(modelId) || { failures: 0, lastFail: '', lastOk: '' }
          if (row.success) {
            if (!stat.lastOk || createdAt > stat.lastOk) stat.lastOk = createdAt
          } else {
            stat.failures += 1
            if (!stat.lastFail || createdAt > stat.lastFail) stat.lastFail = createdAt
          }
          healthByModel.set(modelId, stat)
        }
        for (const [modelId, stat] of healthByModel) {
          if (stat.failures >= HEALTH_DERIVED_DISABLE_FAILURES && stat.lastFail && (!stat.lastOk || stat.lastFail > stat.lastOk)) {
            next.add(modelId)
          }
        }
      }
...
// v2 RPC 返回码 → 响应映射：
//   0 → 通过
//   1 → 429 monthly_quota_exhausted （paid 用户月度+加油包都耗尽）
    ...
    if (code === 1) {
      ...
      return {
        blockedResponse: jsonResponse({
          ...
          message: '本月套餐配额已用完。可购买加油包（¥10 起）或升级 Pro+/Pro Max，下月 1 号 00:00（UTC+8）自动重置。',
          upgrade_url: '/chat/pricing.html',
        }, 429, ch),
        callId,
        tier,
    ...
    const transform = new TransformStream<Uint8Array, Uint8Array>({
      ...
      flush() {
        ...
        if (!lastUsage.tokens_in && !lastUsage.tokens_out) {
          lastUsage = { tokens_in: 0, tokens_out: Math.ceil(outputCharFallback / 2.5), tokens_cached: 0 }
        }
...
async function handleAdminRequest(req: Request, ch: Record<string, string>, body: JsonObject, verifiedUserId: string): Promise<Response> {
  ...
  if (action === 'admin_list_lines') {
    ...
    for (const [id, meta] of Object.entries(SERVER_MODEL_REGISTRY)) {
      ...
      const isAutoDisabled = !dis && disabledSet.has(id)
      ...
      g.lines.push({
        ...
        reason: dis ? dis.reason : (isAutoDisabled ? "自动故障阻断 (近30m失败≥2次)" : null),
        status_code: dis ? dis.status_code : (isAutoDisabled ? 500 : null),
        disabled_at: dis ? dis.disabled_at : (isAutoDisabled ? new Date().toISOString() : null),
        disabled_by: dis ? dis.disabled_by : (isAutoDisabled ? "system_telemetry" : null),
  ...
  if (action === 'admin_enable_line') {
    ...
    // 2. 清除最近 30m 的故障日志，使得自动阻断判定归零，达到强制“解禁”效果
    const since = new Date(Date.now() - HEALTH_DERIVED_DISABLE_WINDOW_MS).toISOString()
  ...
  if (action === 'admin_grant_subscription') {
    ...
    //
    // 2026-05-17 Phase A：可指定 plan_code（'pro' / 'pro_plus' / 'pro_max'），默认 'pro'。
    // 升档逻辑：取当前 plan vs 请求 plan 的 max，monthly_quota 同步升级；
    // monthly_consumed 不重置（同 cancri_activate_paid_code_v2 的策略，防月中升级白嫖）。
    const targetUserId = cleanHeader(String(body.user_id || ''))
    ...
    const baseTs = current?.expires_at && new Date(current.expires_at).getTime() > Date.now()
      ...
      : Date.now()
    const maxExpiry = Date.now() + 90 * 86400 * 1000
    ...
    const newPlan = planRank[requestPlan] > planRank[currentPlan] ? requestPlan : currentPlan
    ...
    // 第一次订阅 → 初始化 monthly_consumed=0 + quota_period_start=NOW
    if (!current) {
      upsertRow.monthly_consumed = 0
      upsertRow.quota_period_start = new Date().toISOString()
    }
    const { error } = await supabase
      .from('user_subscriptions')
      .upsert(upsertRow)
      ...
      if (queueSessionId && (tier !== 'paid' || modelId === GPT55_XHIGH_WELFARE_MODEL_ID)) {
        ...
        try {
          ...
          const wrapped = await wrapResponseForQuotaRecording(
            proxyResp, userId, gate.callId, tier, modelId, gate.costTier,
          )
