...
async function handleArenaRequest(req: Request, ch: Record<string, string>, body: JsonObject, verifiedUserId: string): Promise<Response> {
  ...
  if (action === 'arena_create_match') {
    ...
    const { data, error } = await supabase
      ...
      .select('id,prompt,mode,model_a,model_b,status,created_at,expires_at')
      .single()
    if (error) throw error
    return jsonResponse({
      data: {
  ...
  if (action === 'arena_slot_chat') {
    ...
    const { data: match, error } = await supabase
      .from('arena_matches')
      .select('id,owner_id,mode,model_a,model_b,expires_at')
      .eq('id', id)
  ...
  if (action === 'arena_vote') {
    ...
    const effective = matchMode === 'anonymous' && riskScore < 70
    ...
    try {
      ...
      eloAfter = { modelA: modelAStats.eloScore, modelB: modelBStats.eloScore }
      if (effective) {
        eloAfter = calculateArenaElo(modelAStats.eloScore, modelBStats.eloScore, winner)
