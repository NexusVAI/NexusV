...
function recordModelAvailability(
  ...
  // Hard-disable check first: a single 401/402/quota signal is enough to kill
  // the line for good. Admin must manually re-enable from admin_lines.html
  // after rotating the key in env vars.
  if (shouldAutoDisableLine(status, upstreamText) && !isPersistentAutoDisableProtectedModel(model)) {
    persistDisableLine(model, status, upstreamText);
    // Also park it in memory so the rest of this isolate's requests skip it.
    const entry = modelAvailabilityMap.get(model) || {
      failures: 0,
    ...
    logSecurityEvent("model_auto_disabled", ctx, { model, status });
...
const GEMAI_BASE_URL = (Deno.env.get("GEMAI_BASE_URL") || 'https://api.gemai.cc/v1').replace(/\/$/, "");
...
function pickClawtoApiKey(_model: string): string {
  return CLAWTO_API_KEY_A;
}
...
const PUBLIC_MODEL_ROUTES: Record<string, PublicModelRoute> = {
  ...
  "gemini-3.5-flash-welfare": { upstreamId: "gemini-2.5-flash", brand: "Google", canonicalId: "gemini-3.5-flash-welfare", lineLabel: "aistudio", visible: true, enabled: true, arena: false },
  ...
  "gemini-3-flash-preview": { upstreamId: "gemini-3-flash-preview", brand: "Google", canonicalId: "gemini-3-flash-preview", lineLabel: "aistudio", visible: true, enabled: true, arena: true },
...
function pickPoolProvider(
  ...
  if (model === "gemini-2.5-flash" || model === "gemini-2.5-flash-lite" ||
      model === "gemini-3-flash-preview" || model === "gemma-4-31b-it" ||
  ...
  try {
    ...
    if (endpointName === "image") {
      ...
      if (provider === "iamhc") {
        ...
        headers["Authorization"] = `Bearer ${pickCodeRelayApiKey()}`;
      } else if (provider === "uu6top") {
