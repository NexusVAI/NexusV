...
function checkBanned(ctx: RequestContext, ch: Record<string, string>, userId = ''): Response | null {
  ...
  if (isUserBlocked(userId)) {
    logSecurityEvent('banned_user', ctx, { user: maskIdentifier(userId) })
    return jsonResponse({
      error: 'access_blocked',
      code: 'access_blocked',
      message: '访问被拒绝',
...
const FUTUREPPO_IMAGE_API_KEY = Deno.env.get('FUTUREPPO_IMAGE_API_KEY') || ''
const FUTUREPPO_CLAUDE_API_KEY = Deno.env.get('FUTUREPPO_CLAUDE_API_KEY') || ''
const FUTUREPPO_MODEL_PREFIX = 'futureppo:'

const GEMINI3_BASE_URL = (Deno.env.get('GEMINI3_BASE_URL') || '').replace(/\/\/+$/, '')
const GEMINI3_API_KEY = Deno.env.get('GEMINI3_API_KEY') || ''
const GEMINI3_MODEL_PREFIX = 'gemini3:'
...
function isGemini3Model(model: string): boolean {
  return model.startsWith(GEMINI3_MODEL_PREFIX)
}

function toGemini3ModelId(model: string): string {
  return model.startsWith(GEMINI3_MODEL_PREFIX) ? model.slice(GEMINI3_MODEL_PREFIX.length) : model
}
...
function getFuturePpoApiKey(model: string): string {
  ...
  if (publicModel === 'gpt-image-2' || publicModel === 'glm-5v-turbo') return FUTUREPPO_IMAGE_API_KEY || FUTUREPPO_API_KEY
  if (publicModel === 'claude-sonnet-4-6') return FUTUREPPO_CLAUDE_API_KEY || FUTUREPPO_API_KEY
...
function isNewApiModel(model: string): boolean {
  return model === 'gpt-5.4' || model === 'claude-opus-4.6' || model === 'gemini-2.5-pro'
}
...
function getProviderKind(model: string): ProviderKind {
  if (isGemini3Model(model)) return 'gemini3'
...
function getPingUrl(provider: ProviderKind, probeEndpoint: string): string {
  ...
  if (provider === 'gemini3') {
    return `${GEMINI3_BASE_URL}/chat/completions`
  }
...
const BLOCKED_USERS = parseBlockedUserIds(Deno.env.get('BANNED_USER_IDS') || Deno.env.get('BLOCKED_USER_IDS') || '')
  ...
  try {
    ...
    if (provider === 'futureppo' && (!FUTUREPPO_BASE_URL || !futurePpoApiKey)) {
      ...
      const missingKeyLabel = !futurePpoApiKey
        ? (publicModel === 'gemma-4-31b-chat' ? 'FUTUREPPO_GEMMA_API_KEY or FUTUREPPO_API_KEY'
          : (publicModel === 'gpt-image-2' || publicModel === 'glm-5v-turbo') ? 'FUTUREPPO_IMAGE_API_KEY or FUTUREPPO_API_KEY'
          : publicModel === 'claude-sonnet-4-6' ? 'FUTUREPPO_CLAUDE_API_KEY or FUTUREPPO_API_KEY'
          : 'FUTUREPPO_API_KEY')
        : ''
      return providerConfigError(provider, [
    ...
    if (provider === 'gemini3' && (!GEMINI3_BASE_URL || !GEMINI3_API_KEY)) {
      return providerConfigError(provider, [
        ...(!GEMINI3_BASE_URL ? ['GEMINI3_BASE_URL'] : []),
    ...
    if (provider === 'gemini3' && isLocalOnlyBaseUrl(GEMINI3_BASE_URL)) {
      return new Response(JSON.stringify({ error: 'Invalid provider configuration' }), {
        status: 500,
        headers: { ...ch, 'Content-Type': 'application/json' },
      })
    ...
    if (endpointName === 'image') {
      ...
      if (provider === 'moonshot') {
        ...
        forwardedRequestData = { ...requestData, model: toGemini3ModelId(model) }
      } else if (provider === 'dashscope') {
      ...
      if (provider === 'openai_compatible') {
        ...
        url = `${GEMINI3_BASE_URL}/chat/completions`
        headers['Authorization'] = `Bearer ${GEMINI3_API_KEY}`
      } else {
