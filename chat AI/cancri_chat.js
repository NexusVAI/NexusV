...
const MODEL_CATALOG = [
  ...
  },
  {
    ...
    displayName: "Nemotron 3 Nano 30B Omni",
    brand: "NVIDIA",
    canonicalId: "nemotron-3-nano-omni-30b",
    lineLabel: "线路一",
    visible: true,
    enabled: true,
    arena: true,
  ...
  },
  {
    ...
    brand: "NVIDIA",
    canonicalId: "nemotron-nano-9b-v2",
    lineLabel: "线路一",
    visible: true,
    enabled: true,
    arena: true,
  ...
  },
  {
    ...
    brand: "NVIDIA",
    canonicalId: "nemotron-nano-12b-v2-vl",
    lineLabel: "线路一",
    visible: true,
    enabled: true,
    arena: true,
  ...
  },
  {
    ...
    brand: "Qwen",
    canonicalId: "qwen3-next-80b-a3b",
    lineLabel: "线路一",
    visible: true,
    enabled: true,
    arena: true,
  ...
  },
  {
    ...
    enabled: true,
    arena: true,
    iconPath: "./huggingface-color.svg",
    tags: ["新"],
  },
  {
    id: "laguna-m.1",
  ...
  {
    id: "moonshot-kimi-k2-instruct-dashscope",
    displayName: "Moonshot-Kimi-K2-Instruct",
    brand: "Kimi",
    canonicalId: "moonshot-kimi-k2-instruct",
    lineLabel: "线路二",
  ...
  {
    id: "qwen3.6-flash-2026-04-16-dashscope",
    displayName: "Qwen3.6 Flash",
    brand: "通义千问",
    canonicalId: "qwen3.6-flash-2026-04-16",
    lineLabel: "线路二",
  ...
  // ── mimo (小米) ──
  {
    ...
    enabled: true,
    arena: false,
    iconPath: "./xiaomimimo-color.svg",
    tags: ["长程任务", "推理"],
  },
].sort((a, b) => {
  const rankA = MODEL_PRIORITY.has(a.id)
    ? MODEL_PRIORITY.get(a.id)
    : MODEL_DEPRIORITY.has(a.id)
      ? 2000 + MODEL_DEPRIORITY.get(a.id)
      : 1000;
  const rankB = MODEL_PRIORITY.has(b.id)
    ? MODEL_PRIORITY.get(b.id)
    : MODEL_DEPRIORITY.has(b.id)
      ? 2000 + MODEL_DEPRIORITY.get(b.id)
      : 1000;
  return rankA - rankB;
});
...
const ENABLED_MODEL_CATALOG = MODEL_CATALOG.filter(
  (model) => model.enabled !== false,
);
const SELECTABLE_MODELS = MODEL_CATALOG.filter(
  (model) => model.visible !== false && model.enabled !== false,
);
const ARENA_MODELS = SELECTABLE_MODELS.filter(
  (model) => model.arena !== false && !String(model.id).startsWith("image-"),
);
const MODEL_IDS = Object.fromEntries(
  ENABLED_MODEL_CATALOG.map((model) => [model.id, model.id]),
);
const MULTIMODAL_MODEL_IDS = new Set(
  ENABLED_MODEL_CATALOG.filter((model) => model.multimodal).map(
    (model) => model.id,
  ),
);
...
function isModelSelectable(modelId) {
  ...
  if (meta.visible === false || meta.enabled === false || !MODEL_IDS[modelId])
    return false;
  if (
    (meta.imageOnly || meta.videoOnly) &&
...
function getFallbackModelId(excludeId = "") {
  const model = SELECTABLE_MODELS.find(
    (item) => item.id !== excludeId && isModelEnabled(item.id),
  );
...
function getModelBrandName(modelId) {
  ...
  const text =
    `${meta.id || ""} ${meta.displayName || ""} ${meta.iconPath || ""}`.toLowerCase();
...
const savedModelSelection =
  localStorage.getItem("cancri_current_model") || DEFAULT_MODEL_ID;
let currentModel = resolveSelectableModelId(
  savedModelSelection,
  DEFAULT_MODEL_ID,
);
...
const savedCompareModelSelection =
  localStorage.getItem("cancri_compare_model") || DEFAULT_COMPARE_MODEL_ID;
let compareModel = resolveSelectableModelId(
  savedCompareModelSelection,
  ...
  if (modelDropdown) {
    ...
    if (triggerEl) {
      ...
      if (isCompact) {
        ...
        // Width: 420-520px but capped so it fits in the viewport
        const maxW = Math.max(240, vw - 24);
        const width = Math.round(
          Math.min(520, Math.max(Math.min(420, maxW), rect.width)),
        );
        const top = Math.round(rect.bottom + 8);
        // Right-align when button center is past viewport midpoint
        // (natural for header buttons on the right side)
        const buttonCenterX = rect.left + rect.width / 2;
        if (buttonCenterX > vw / 2) {
          const right = Math.max(12, vw - rect.right);
          modelDropdown.style.top = `${top}px`;
          modelDropdown.style.left = "auto";
          modelDropdown.style.right = `${right}px`;
        } else {
