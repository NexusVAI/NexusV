    ...
    news3: {
        ...
        zh: {
            ...
            paragraphs: [
                ...
                'ChatAI 服务作为生成式人工智能聚合平台，其输出内容由第三方 AI 模型基于用户输入自动生成。开发者运营云端网关进行请求转发、配额管控与用量记录，不参与 AI 模型的实际推理过程，不对任何特定输出内容拥有著作权或编辑控制权。用户使用 ChatAI 服务生成的内容，应自行判断其合法性、准确性与适用性，并自行承担因使用该等内容所产生的法律后果。用户不得利用 ChatAI 服务生成违反当地法律法规的内容，包括但不限于虚假信息、侵权内容、违法有害信息等。开发者保留在发现违法违规使用行为时限制或终止用户访问 ChatAI 服务的权利。',
