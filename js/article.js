const articleData = {
    ...
    news3: {
        ...
        zh: {
            ...
            paragraphs: [
                '重要提示（建议先阅读此部分）',
                '本套件默认使用本地部署的SFT微调Qwen模型（自带一键安装器），强烈推荐本地运行，默认模式下不向任何服务器发送数据。',
                ...
                '本协议自 2026 年 4 月 13 日起生效。先前版本以其原生效规则处理。',
                '本使用协议（以下简称"本协议"）适用于由 某宇（以下简称"开发者"）开发并发布的 NexusV套件（以下简称"本套件"）。在下载、安装或以任何方式使用本套件前，请您务必仔细阅读并理解本协议的全部内容。凡以任何方式使用本套件，均视为您已充分阅读、理解并同意接受本协议的全部条款；若您不同意本协议任何条款，请立即停止使用并删除本套件及其所有相关文件。',
                '1. 定义与说明',
                '"本套件"指 NexusV系列，包括：(1) TACTFR（警察玩法增强模块，包含娱乐性质的AI角色扮演功能，仅限娱乐）；(2) NexusV修改器（单机线下游戏数值修改）；(3) Sentience（AI驱动NPC，可本地或用户自备Key调用云端大模型）。',
                'TACTFR、NexusV修改器与Sentience具有相对独立的功能模块和风险边界，适用本协议中与其功能相对应的条款。',
                '"第三方组件"指本套件中嵌入或引用的由其他作者/组织提供的库、工具或二进制文件（详见第 3 节）。',
                '本套件为第三方非官方产品，未获下列原版权方授权或认可：Grand Theft Auto V、Rockstar Games、及 Take-Two Interactive。本套件仅为技术演示与娱乐用途，不构成上述任何方的官方扩展或联名产品。',
                '2. 授权与许可声明',
                '2.1 Sentience全部源代码以 MIT 许可证开源，见仓库 LICENSE。您可自由使用/修改，但必须同时遵守所用模型的原始许可。',
                '2.2 TACTFR与NexusV修改器原创部分保留所有权利，仅授权个人、非商业、单机线下使用。',
                '2.3 许可范围说明：Sentience全部源代码以 MIT 许可证开源，其单独源代码的使用、修改和分发以 MIT 许可证为准。但本套件整体（包括 TACTFR、NexusV 修改器与 Sentience 的整合打包版本）仅授权个人、非商业、单机线下娱乐使用。未经开发者书面授权，禁止将本套件用于任何商业目的、付费分发或盈利行为。',
                '第三方组件权利以其原始许可为准，本协议不得限制。',
                '3. 第三方组件与AI模型',
                '本套件使用：ScriptHookV（不随包分发，请用户自行官网下载）、ScriptHookVDotNet、NAudio、Newtonsoft.Json。',
                '本套件仅作为脚本补丁运行，不包含、不修改、不分发任何原游戏（GTA V）的核心二进制文件。用户需自行承担因安装 ScriptHookV 等第三方注入工具导致的软件冲突或系统不稳定风险。',
                'Sentience默认模型：基于Qwen系列的SFT微调权重（如Qwen2.5/Qwen3），遵循其原始模型许可（详见模型文件夹内LICENSE），您使用即视为同意该许可。',
                '云端模式需用户自备：OpenAI API、DeepSeek API 等，使用即视为您已同意其服务条款与AUP。LM Studio为可选本地工具。',
                '4. 隐私与数据处理',
                '4.1 默认本地模式（强烈推荐）：Sentience使用内置的SFT微调Qwen模型，全部推理在用户本地完成，不会向任何第三方发送游戏数据或玩家输入。开发者不收集任何个人数据。',
                '4.2 云端API模式（需用户主动配置）：用户在本地config.ini中填入自己的DeepSeek、OpenAI等API Key后，本套件会将NPC对话上下文直接发送至用户指定的服务商。开发者不中转、不存储、不查看用户的API Key或对话内容。API Key以明文存储在本地用户可访问的配置文件中，请勿分享配置文件或上传至公开仓库，强烈建议优先使用本地模式以避免任何数据传输和费用风险。使用云端模式时，您的IP地址、设备信息将由您的电脑直接发送给所选API服务商，受其隐私政策约束。开发者不作为数据处理者。',
                '4.3 数据最小化：仅发送必要上下文，不会故意发送真实姓名、身份证号、支付信息等敏感个人信息。严禁在与 Sentience 的对话中输入真实姓名、身份证号、住址、电话号码、支付信息等高度敏感个人信息，否则造成的一切后果（包括但不限于信息泄露、被模型记忆、法律风险等）均由用户自行承担。',
                '4.4 日志：本版本无开发者服务器，因此开发者不保留任何云端日志。如未来推出自建云端推理平台，将按照《生成式人工智能服务管理暂行办法》等法律法规要求另行公告并配套单独服务协议与隐私政策。用户本地日志可自行删除。',
                '本协议同时遵守《中华人民共和国个人信息保护法》（PIPL）。',
                '5. AI 生成内容与治理',
                'Sentience功能遵守《生成式人工智能服务管理暂行办法》《互联网信息服务深度合成管理规定》等相关法律法规。本套件当前以本地部署为主，如未来提供面向公众的云端服务，将依法评估并履行相应合规义务。',
                'AI输出的独立性：本套件所生成的所有对话、故事情节、观点，系由所用模型即时生成，仅代表模型输出，不构成开发者或任何第三方的立场或声明。',
                '不可预测性：模型输出具有不确定性，可能包含错误、虚构、冒犯性或不适当内容；开发者无法且不对模型在任意时刻的具体输出承担完全可预见的责任。模型生成的任何有关医疗、法律、投资等专业领域的建议均为虚构，严禁将其视为现实参考。',
                '内容过滤与上报机制：开发者已在默认本地模型中内置内容过滤机制（仅为辅助性风险控制措施，不构成对输出内容合法性、准确性或适当性的保证），并强烈推荐使用本地模式以降低风险，但不能保证完全拦截所有不当内容。请及时通过邮箱（见第10节）上报违法或严重冒犯内容。',
                '深度合成标识：Sentience生成的NPC对话属于AI合成内容，游戏内未做强制水印。您在分享相关视频时，必须在简介或视频显著位置标注"本视频包含AI合成对话/虚构内容"，否则由此引发的一切监管风险由您自行承担。您不得将游戏录屏/对话对外传播时冒充真人、冒充官方、或用于新闻、政务等可能误导公众的场景。',
                ...
                '6. 使用限制与禁止行为',
                '年龄限制：本套件基于限制级（18+）游戏运行，且涉及数据联网处理。您必须是年满 18 周岁的完全民事行为能力人方可使用本套件。若您未满法定年龄，请立即停止使用。',
                '严禁在GTA Online或任何Rockstar官方多人服务中使用本套件任何功能，由此导致的封号由您自负。',
                '禁止将本套件用于任何违法用途或以任何方式进行商业变现（如未经授权的付费分发、代售等），除非事先获得开发者书面授权。',
                '除非第三方组件原始许可允许，您不得对开发者的专有部分实施逆向工程、反编译或反汇编（Sentience按MIT许可除外）。对以开源许可分发的组件，请按其原始许可执行。',
                ...
                '不担保可用性：开发者不保证本套件将持续不间断、安全、无错误或完全符合用户期望；也不保证任何第三方服务或模型的可用性与稳定性。',
                '风险自负：用户明确知悉并同意，因下载、安装或使用本套件所可能导致的游戏账号风险（包括但不限于被官方封禁）、游戏存档损坏或丢失、数据丢失、系统损坏或其他任何损失，均由用户自行承担。开发者不对上述损失承担责任。',
                '责任限额：在适用法律允许的最大范围内，开发者对因本套件引起或与之相关的任何直接损害、间接损害、特殊损害、惩罚性损害或后果性损害不承担责任。若在某一司法管辖区法律不允许完全排除责任，则开发者对可归责责任的总体赔偿金额以法律允许的最大范围为限；若用户未向开发者支付任何费用，则最高赔偿责任上限为人民币100元（取其高者），但不影响法律规定不得排除或限制的责任。本条款不适用于故意或重大过失，不适用于法定不可免责责任。',
                'API 密钥安全：您的API Key以明文形式保存在本地config.ini中（技术限制无法加密），请勿分享该文件，不要在公共电脑或多人共用的设备上配置云端API功能。开发者不会在日志或崩溃报告中主动收集您的Key，但无法阻止您自行泄露。因泄露、被盗刷产生的费用由您全责承担。',
                ...
                '8. 维护、更新与协议变更',
                '开发者可不定期对本套件进行更新、维护或功能变更。对于本协议的修改，开发者将在模组发布页/仓库主页发布变更说明并标注生效日期。对于影响用户权利义务的重大变更，开发者将通过发布页公告及首次启动弹窗提示，并在公告中说明变更要点及生效日。继续使用本套件即视为接受更新后的协议。',
                '若用户不同意任何变更，应停止使用并删除本套件。',
                '9. 知识产权',
                '9.1 Sentience源代码著作权归开发者，其使用受MIT许可证约束，您可按MIT条款自由复制、修改、分发。',
                '9.2 TACTFR与NexusV修改器的原创部分保留所有权利，仅授权个人非商业使用，未经书面许可不得复制、修改或再分发。',
                ...
                '邮箱：nexusv@139.com',
                '请勿通过邮件或issue直接发送API Key、密码、身份证号等敏感信息。',
                '或在模组发布仓库提交 issue（若使用 GitHub/GitLab，请使用发布页提供的 issue 模板）。',
                '开发者将在合理时间内确认收悉并在可行范围内处理或给出处理计划。若涉及法律问题，开发者保留将相关信息移交执法或权利人处理的权利。',
                '11. 终止',
                '若用户违反本协议之任一条款，开发者有权立即终止向该用户提供服务并要求其删除本套件全部副本；同时开发者可采取进一步的技术或法律措施维护合法权益。',
                '因版权方要求的终止：鉴于本套件依附于《Grand Theft Auto V》，若收到原版权方（Take-Two Interactive 或 Rockstar Games）的停止通知（Cease & Desist）或相关维权要求，开发者有权随时、无条件且不经提前通知地永久停止本套件的维护、下架相关文件并终止云端推理服务（若有），且无需向用户承担任何违约或赔偿责任。',
                ...
                '本协议构成您与开发者之间关于本套件之完整协议，取代此前任何口头或书面的协商与协议（但不影响对第三方组件原始许可的约束力）。'
            ]
        },
        en: {
            ...
            paragraphs: [
                ...
                'Sentience core code is fully open source.',
                ...
                'This Agreement is effective as of April 13, 2026. Previous versions shall be governed by their original effective terms.',
                'This Terms of Use Agreement ("Agreement") applies to the NexusV Suite developed and published by 某宇 ("Developer").',
                '1. Definitions',
                '"Suite" refers to the NexusV series, including: (1) TACTFR (police gameplay enhancement module, featuring entertainment-oriented AI role-play functions, for entertainment only); (2) NexusV Modifier (single-player offline game value modification); (3) Sentience (AI-driven NPCs, can run locally or call cloud LLMs with user-provided API Key).',
                'TACTFR, NexusV Modifier, and Sentience have relatively independent functional modules and risk boundaries, applicable to corresponding terms in this Agreement.',
                '"Third-Party Components" means libraries, tools, or binaries from other authors/organizations embedded in or referenced by the Suite (see Section 3).',
                'The Suite is an unofficial third-party product, not authorized or endorsed by Grand Theft Auto V, Rockstar Games, or Take-Two Interactive.',
                '2. License Grant',
                '2.1 Sentience full source code is open source under MIT License, see repository LICENSE. You may freely use/modify, but must comply with the original license of the model used.',
                '2.2 TACTFR and NexusV Modifier original parts retain all rights, licensed only for personal, non-commercial, single-player offline use.',
                '2.3 License Scope: Sentience full source code is open source under MIT License, its use, modification and distribution of individual source code follow MIT License. However, the overall Suite (including integrated packaged versions of TACTFR, NexusV Modifier and Sentience) is licensed only for personal, non-commercial, single-player offline entertainment use. Without prior written authorization from Developer, it is prohibited to use the Suite for any commercial purpose, paid distribution or profit-making activities.',
                ...
                '4.2 Cloud API Mode (Requires User Initiative): After you enter your own API Key in local config.ini, the Suite sends NPC dialogue context directly to your designated service provider. Developer does NOT intercept, store, or view your API Key or dialogue content. API Key is stored in plain text in locally accessible config file, do NOT share this file or upload to public repositories, strongly recommend using local mode to avoid data transmission and cost risks. When using cloud mode, your IP address and device info will be sent directly to your chosen API service provider by your PC, governed by their privacy policy. Developer is not a data processor.',
                '4.3 Data Minimization: Only sends necessary context, will NOT intentionally send real name, ID number, payment info or other sensitive personal information. It is strictly prohibited to input highly sensitive personal information such as real name, ID number, address, phone number, payment info, etc. in conversations with Sentience. Any consequences arising therefrom (including but not limited to information leakage, model memorization, legal risks, etc.) shall be borne solely by the user.',
                '4.4 Logs: This version has no Developer server, so Developer does NOT retain any cloud logs. If self-built cloud inference platform is provided in the future, it will be announced separately with separate service agreement and privacy policy in compliance with relevant laws. Local logs on your PC can be deleted by yourself.',
                'This Agreement also complies with the Personal Information Protection Law (PIPL) of China.',
                '5. AI-Generated Content & Governance',
                'Sentience features comply with the Interim Measures for the Management of Generative Artificial Intelligence Services, Provisions on the Management of Deep Synthesis of Internet Information Services and other relevant laws/regulations. This Suite focuses on local deployment; if public cloud services are provided in the future, Developer will evaluate and fulfill corresponding compliance obligations as required by law.',
                ...
                'Prohibited Uses: It is strictly forbidden to use the Suite to generate, store, disseminate, or induce any content that violates local laws and regulations, including but not limited to pornography, violent terrorism, hate speech, privacy infringement, or defamation. Developer reserves the right to take technical measures or pursue legal liability for malicious abuse.',
                '6. Usage Restrictions',
                'Age Restriction: The Suite is built upon a mature-rated (18+) game and involves online data processing. You must be at least 18 years old (or the legal age of majority in your jurisdiction) and have full civil capacity to use the Suite. If you are under the legal age, please stop using it immediately.',
                'STRICTLY PROHIBITED from using any part of this Suite in GTA Online or any Rockstar official multiplayer service. Any resulting account bans are your sole responsibility.',
                ...
                '9.2 TACTFR and NexusV Modifier original parts retain all rights, licensed only for personal non-commercial use, without prior written permission, shall not copy, modify or redistribute.',
                ...
                'If you discover security issues, illegal or seriously inappropriate AI outputs during use, or need to exercise data query/deletion rights, please contact Developer through the following methods (please note "NexusV — Issue Report/Data Request" in the email subject):',
