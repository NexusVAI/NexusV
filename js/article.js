const articleData = {
    ...
    tactfr540: {
        overlay: '',
        media: { type: 'image', src: 'Logo/H11.webp', alt: 'TACTFR 5.5.0' },
        zh: {
            title: '震撼推出 TACTFR 5.5.0',
            date: '2026/03/10',
            category: '产品',
            readTime: '10 分钟阅读',
            paragraphs: [
                '在 TACTFR 的演进逻辑中，每一个大版本的跳跃都代表着我们对"真实感"理解维度的扩张。如果说 5.4.0 版本是关于稳定性与确定性的基石，那么 5.5.0 Beta 则标志着我们开始探索虚拟执法中最具挑战性的领域：多目标协同与复杂动态交互。',
                '1. 跨越维度的"双人案件"系统',
                '在现实的执法环境中，警员面对的极少是孤立的个体。5.5.0 Beta 正式引入了"双人案件（Dual-Person Cases）"架构。这不仅仅是实体的简单增加，更是底层逻辑从"单一状态机"向"多主体协作网络"的跃迁。',
                '全场景覆盖：新系统支持"步行巡逻"与"车辆截停"两大核心品类。',
                '交互涌现性：当目标增加到两人时，玩家必须处理更复杂的优先级判断——如何同时控制两名嫌疑人？如何应对一人服从、另一人试图逃离的突发状况？',
                '逻辑连贯性：我们重构了案件接取终端，让这种多目标任务能够无缝嵌入现有的巡逻流程中。',
                '2. 终端交互的"触感"进化',
                '工具的响应速度决定了沉浸感的深度。在 5.5.0 Beta 中，我们针对终端操作进行了深度性能优化与手感调校。',
                '通过对输入反馈链路的重构，终端的操作不再仅仅是"点击"，而是一种具备物理直觉的交互。无论是案件筛选还是指令下达，系统现在的响应更加灵敏、丝滑，消除了视觉与操作之间的微小延迟。',
                '3. 5.4.0 的遗产：建立在稳定性之上的实验',
                '5.5.0 的激进尝试，离不开 5.4.0 版本打下的坚实地基。回顾 5.4.0，我们通过一系列底层的"大扫除"，确保了整个系统的健康：',
                '循环闭环：修复了案件无法终止的 Bug，确保每一场追逐都有其逻辑终点。',
                '资源释放：优化性能开销，彻底解决了高负载下的"卡死"痛点。',
                '状态纯净度：消除了拘捕动画被错误继承的"鬼影"Bug。',
                '包容性设计：通过 TACTFR.ini 实现了全按键自定义，并引入了极具氛围感的电台语音系统。',
                '正是这些看不见的稳定性工作，支撑起了 5.5.0 对复杂案件类型的探索。',
                '4. Beta 阶段：迭代中的协作',
                '作为一个包含重大架构调整的版本，5.5.0 Beta 是一次前沿性的探索。我们深知，在复杂的沙盒环境中，多实体交互可能产生意料之外的边界案例。',
                '部署策略：5.5.0 目前处于 Beta 阶段，旨在收集极端环境下的运行数据。',
                '回滚机制：若在体验过程中遇到逻辑冲突或不稳定的情况，我们建议玩家进群反馈，并可随时卸载重装 5.4.0 稳定版，直至后续完善版发布。',
                '反馈渠道：您的每一份 Bug 报告，都是 Sentience-LS 与 TACTFR 进化过程中的关键样本。',
                '5. 结语',
                '从解决一个人的服从，到处理两个人的博弈，5.5.0 Beta 正在将 TACTFR 推向一个更具社会学深度的模拟阶段。洛圣都的街头不再是单一目标的练习场，而是一个充满变数、更接近真实法律边界的实验室。',
                '探索已经开启，挑战现已就绪。',
                '"复杂性不是障碍，它是通往真实的必经之路。"',
                '[BUG 反馈群：1079691553]',
                '[点击页面右上角"使用 NexusV 即可下载"]'
            ]
        },
        en: {
            title: 'Introducing TACTFR 5.5.0',
            date: '2026/03/10',
            category: 'Product',
            readTime: '10 min read',
            paragraphs: [
                'In TACTFR\'s evolution logic, every major version leap represents an expansion of our understanding of "realism." If version 5.4.0 was the cornerstone of stability and certainty, then 5.5.0 Beta marks the beginning of our exploration into the most challenging domain of virtual law enforcement: multi-target coordination and complex dynamic interactions.',
                '1. The "Dual-Person Cases" System Across Dimensions',
                'In real-world law enforcement environments, officers rarely face isolated individuals. 5.5.0 Beta officially introduces the "Dual-Person Cases" architecture. This is not merely a simple increase in entities, but a leap in underlying logic from "single state machine" to "multi-agent collaborative network."',
                'Full Scene Coverage: The new system supports two core categories: "Foot Patrol" and "Vehicle Stop."',
                'Emergent Interactions: When targets increase to two, players must handle more complex priority judgments—how to control two suspects simultaneously? How to respond to sudden situations where one complies while the other attempts to flee?',
                'Logical Coherence: We have reconstructed the case acquisition terminal, allowing such multi-target missions to seamlessly integrate into existing patrol workflows.',
                '2. The "Tactile" Evolution of Terminal Interactions',
                'Tool response speed determines the depth of immersion. In 5.5.0 Beta, we have conducted deep performance optimization and feel tuning for terminal operations.',
                'Through reconstruction of the input feedback chain, terminal operations are no longer mere "clicks," but interactions with physical intuition. Whether filtering cases or issuing commands, the system now responds more sensitively and smoothly, eliminating the tiny delay between vision and operation.',
                '3. The Legacy of 5.4.0: Experiments Built on Stability',
                'The radical attempts of 5.5.0 rely on the solid foundation laid by version 5.4.0. Looking back at 5.4.0, we ensured the health of the entire system through a series of underlying "cleanups":',
                'Loop Closure: Fixed bugs where cases could not terminate, ensuring every chase has its logical endpoint.',
                'Resource Release: Optimized performance overhead, completely solving the "freezing" pain point under high load.',
                'State Purity: Eliminated the "ghost" bug where arrest animations were incorrectly inherited.',
                'Inclusive Design: Achieved full key customization through TACTFR.ini, and introduced an atmospheric radio voice system.',
                'It is these invisible stability efforts that support 5.5.0\'s exploration of complex case types.',
                '4. Beta Phase: Collaboration in Iteration',
                'As a version containing major architectural adjustments, 5.5.0 Beta is a cutting-edge exploration. We are well aware that in complex sandbox environments, multi-entity interactions may produce unexpected edge cases.',
                'Deployment Strategy: 5.5.0 is currently in Beta, aimed at collecting operational data in extreme environments.',
                'Rollback Mechanism: If you encounter logical conflicts or instability during experience, we recommend joining the feedback group. You can also uninstall and reinstall the 5.4.0 stable version at any time until a subsequent refined version is released.',
                'Feedback Channel: Every bug report you submit is a key sample in the evolution of Sentience-LS and TACTFR.',
                '5. Conclusion',
                'From solving one person\'s compliance to handling the game between two people, 5.5.0 Beta is pushing TACTFR toward a simulation stage with greater sociological depth. The streets of Los Santos are no longer a practice ground for single targets, but a laboratory full of variables, closer to real legal boundaries.',
                'Exploration has begun, challenges are now ready.',
                '"Complexity is not an obstacle; it is the necessary path to reality."',
                '[BUG Feedback Group: 1079691553]',
                '[Click "Use NexusV" in the upper right corner to download]'
            ]
        }
    },
    n4: {
    ...
    sentienceLS: {
        ...
        zh: {
            ...
            paragraphs: [
                ...
                '路线图（未来几周）',
                '2天内 ：稳定本地训练脚本、完善 smoke-test 流程、发布示例数据集（200 条 GTA 风格样本）。',
                '1~2周内 ：支持更多导出格式、优化量化/推理脚本、提供一键部署到本地 REST 服务的示例。',
                '1月内 ：推出角色市场（可分享 LoRA adapter）、提供工具链自动化（数据增强、质量检测、风格一致性评估）。',
                '安全与责任',
        ...
        en: {
            ...
            paragraphs: [
                ...
                'Within 2 days: Stabilize local training scripts, improve smoke-test workflow, release sample dataset (200 GTA-style samples).',
                'Within 1-2 weeks: Support more export formats, optimize quantization/inference scripts, provide one-click deployment examples to local REST services.',
                'Within 1 month: Launch character marketplace (shareable LoRA adapters), provide toolchain automation (data augmentation, quality detection, style consistency evaluation).',
    ...
    news3: {
        ...
        zh: {
            ...
            paragraphs: [
                ...
                '1. 定义与说明',
                '"本模组"指 NexusV 及其相关文件、安装包、插件、脚本、配置和随附文档。',
                '"第三方组件"指本模组中嵌入或引用的由其他作者/组织提供的库、工具或二进制文件（示例详见第 3 节）。',
                '本模组为第三方非官方产品，未获下列原版权方授权或认可：Grand Theft Auto V、Rockstar Games、及 Take-Two Interactive。本模组仅为技术演示与娱乐用途，不构成上述任何方的官方扩展或联名产品。',
                '2. 授权与许可声明',
                '本模组包含开发者原创代码与若干第三方组件。第三方组件各自受其原始许可约束；开发者不对第三方组件之原始许可内容做出更改或额外限制。请用户在使用本模组前，查阅并遵守对应第三方组件的许可条款。',
                '开发者对其原创部分可另行设定许可（例如"非商业使用"或其他限制），该等许可条款应在相应文件中明确列出并与本协议共同生效。若第三方组件许可与开发者对原创部分之许可存在冲突，以第三方组件的原始许可为准（第三方组件在其许可范围内的权利不得被本协议单方面剥夺或限制）。',
                '3. 第三方组件（示例与合规提示）',
                '本模组可能会引用或使用下列第三方库（示例）：',
                'ScriptHookV；',
                'ScriptHookVDotNet；',
                'NAudio；',
                'Newtonsoft.Json。',
                ...
                '日志与保留期：本模组在默认情况下可能会保留服务日志与对话记录，用于错误排查与模型性能优化。默认日志保留期为 30 天（除非另行说明）。用户可通过第 10 节联系方式申请查询或删除其个人数据（删除请求将在合理时间内处理）。',
                '选择与同意：若本模组具备"本地/离线模型"或"禁用联网推理"选项，用户可启用该选项以避免将数据发送至云端。若用户启用联网推理或配置第三方 API key，则视为用户已明确同意相应的数据传输与处理。开启任何第三方服务即表示用户同时应遵守该第三方的隐私政策与服务条款。',
                '合规性提示：若您位于适用数据保护法规（例如 GDPR 或其他当地隐私法规）管辖区，请在启用联网功能前确认您的使用行为与本地法规保持一致，必要时请先取得相关同意。',
                '5. AI 生成内容与治理',
                'AI 输出的独立性：本模组所生成的所有对话、故事情节、观点或所谓"觉醒"言论，系由所用模型即时生成，仅代表模型输出，不构成开发者或任何第三方的立场或声明。',
                '不可预测性：模型输出具有不确定性，可能包含错误、虚构、冒犯性或不适当内容；开发者无法并不对模型在任意时刻的具体输出承担完全可预见的责任。',
                ...
                '若用户不同意任何变更，应停止使用并删除本模组。',
                '9. 知识产权',
                '本模组中开发者原创的源代码、资源与文档的著作权归开发者所有（除非另行声明）。未经开发者书面许可，不得擅自复制、修改、再分发或用于商业目的（但需尊重并遵守已声明为开源的部分及其原始许可）。',
                '本模组中引用之第三方商标、游戏资产与品牌之知识产权归原权利人所有，用户不得主张任何所有权或以任何方式暗示官方授权。',
                '10. 投诉、问题反馈与联系信息',
                '若您在使用过程中发现安全问题、违法或严重不当的 AI 输出、或需行使数据查询/删除权利，请通过下列方式与开发者联系（请在邮件主题注明"NexusV — 问题上报/数据请求"）：',
                '邮箱：nexusv@139.com',
                '或在模组发布仓库提交 issue（若使用 GitHub/GitLab，请使用发布页提供的 issue 模板）。',
                ...
                '因本协议产生的争议，双方应首先友好协商；协商不成的，任何一方可向开发者所在地有管辖权的人民法院提起诉讼。',
                '13. 其他',
                '本协议各条款如因任何原因被有管辖权之法院判定为全部或部分无效，不影响其他条款的效力。',
                '本协议构成您与开发者之间关于本模组之完整协议，取代此前任何口头或书面的协商与协议（但不影响对第三方组件原始许可的约束力）。'
            ]
        },
        en: {
            ...
            paragraphs: [
                ...
                '1. Definitions',
                '"Mod" refers to NexusV and all related files, installers, plugins, scripts, configurations, and documentation.',
                '"Third-Party Components" means libraries, tools, or binaries from other authors/organizations embedded in or referenced by the Mod (see Section 3 for examples).',
                'The Mod is an unofficial third-party product, not authorized or endorsed by Grand Theft Auto V, Rockstar Games, or Take-Two Interactive.',
                '2. License Grant',
                'The Mod contains original code by the Developer and various Third-Party Components. Each Third-Party Component is subject to its original license. Users must comply with all applicable third-party license terms.',
                'The Developer may set additional licenses for original parts (e.g., "non-commercial use" or other restrictions). Such license terms shall be clearly listed in corresponding documents and take effect together with this Agreement. If there is a conflict between third-party component licenses and the Developer\'s licenses for original parts, the original licenses of third-party components shall prevail.',
                '3. Third-Party Components',
                'The Mod may include the following third-party libraries (examples):',
                'ScriptHookV;',
                'ScriptHookVDotNet;',
                'NAudio;',
                'Newtonsoft.Json.',
                ...
                'Log Retention: The Mod may retain service logs and dialogue records by default for error troubleshooting and model performance optimization. The default log retention period is 30 days (unless otherwise specified). Users may request to query or delete their personal data through the contact information in Section 10 (deletion requests will be processed within a reasonable time).',
                'Consent: If the Mod has "local/offline model" or "disable online inference" options, users may enable them to avoid sending data to the cloud. If users enable online inference or configure third-party API keys, it is deemed that users have explicitly agreed to corresponding data transmission and processing. Enabling any third-party service means users should also comply with that third party\'s privacy policy and terms of service.',
                'Compliance: If you are in a jurisdiction subject to data protection regulations (e.g., GDPR or other local privacy regulations), please confirm that your use is consistent with local regulations before enabling online features, and obtain relevant consent if necessary.',
                '5. AI-Generated Content',
                'Independence of AI Outputs: All dialogues, storylines, opinions, or so-called "awakening" remarks generated by the Mod are generated in real-time by the model used and represent only model outputs, not positions or statements of the Developer or any third party.',
                'Unpredictability: Model outputs are uncertain and may contain errors, fictional, offensive, or inappropriate content; the Developer cannot and does not assume fully foreseeable responsibility for specific outputs of the model at any given time.',
                ...
                'If users do not agree to any changes, they should stop using and delete the Mod.',
                '9. Intellectual Property',
                'The copyright of original source code, resources, and documentation in the Mod belongs to the Developer (unless otherwise stated). Without written permission from the Developer, copying, modification, redistribution, or use for commercial purposes is not allowed (but you must respect and comply with parts declared as open-source and their original licenses).',
                'The intellectual property rights of third-party trademarks, game assets, and brands cited in the Mod belong to the original rights holders, and users shall not claim any ownership or imply official authorization in any way.',
                '10. Complaints, Feedback & Contact Information',
                'If you discover security issues, illegal or seriously inappropriate AI outputs during use, or need to exercise data query/deletion rights, please contact the Developer through the following methods (please note "NexusV — Issue Report/Data Request" in the email subject):',
                'Email: nexusv@139.com',
                'Or submit an issue on the Mod release repository (if using GitHub/GitLab, please use the issue template provided on the release page).',
                ...
                'This Agreement constitutes the complete agreement between you and the Developer regarding the Mod, superseding any prior oral or written negotiations and agreements (but does not affect the binding force of original licenses of third-party components).'
            ]
        }
    },
    news4: {
