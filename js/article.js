const articleData = {
    ...
    ccAcademy: {
        ...
        zh: {
            title: 'CancriCode Academy：从第一次对话到第一次交付',
            date: '2026年9月26日',
            category: '学院',
            readTime: '8 分钟阅读',
            paragraphs: [
                '很多人第一次打开 Cancri Code，会像打开一个聊天窗口那样，问它一个问题，然后等一个答案。这没有错，但只用到了它十分之一的能力。Academy 想做的事情很简单：帮你把这个习惯，换成"把一件事交出去"。',
                '<strong>第一课：先把项目交给它</strong>',
                '安装完成、登录账号之后，打开你正在做的那个文件夹。不需要额外配置环境，也不需要先写一份项目说明。Cancri Code 会自己去读目录结构、找入口、看依赖——就像一位新同事入职第一天会做的那样。你要做的只是回答它偶尔提出的问题。',
                '<strong>第二课：说目标，而不是说步骤</strong>',
                '"把第 42 行改成 xxx"是在指挥一支笔；"登录页在弱网下会卡住，帮我找到原因并修掉"才是在交付一件事。前者你得自己知道答案，后者才是 Cancri Code 存在的意义。描述你想要的结果、你判断"做好了"的标准，剩下的交给它去查、去试、去验证。',
                '<strong>第三课：学会看它的工作过程</strong>',
                '每一次读文件、搜索、执行命令、修改代码，都会原样出现在对话里。你不需要逐条审阅，但请养成一个习惯：在它准备写入文件或执行有副作用的操作时，看一眼它打算做什么。审批不是负担，是你对自己代码库的最后一道把关。',
                '<strong>第四课：把高风险的事放进云沙箱</strong>',
                '压测、跑陌生的脚本、装一堆你不想污染本机的依赖、打开一个真实浏览器去点一遍流程——这些事，交给云端沙箱。每个任务一台隔离的机器，用完就收回，不碰你的电脑。我们和派欧云（PPIO）一起把这件事做到了按需启动、闲置自动暂停，你只为真正用到的时间付费。',
                '<strong>第五课：选对档位</strong>',
                '不是每件事都值得让模型想很久。改个文案、查个函数在哪，用低档位，快而省；跨模块重构、排查诡异的并发问题，就把档位拉高，让它多想、多验证。档位是你在速度和深度之间做的取舍，而这个取舍，只有你最清楚。',
                '<strong>接下来</strong>',
    ...
    ccBestPractice: {
        ...
        media: { type: 'image', src: 'Logo/Cancri0715.png', alt: 'Cancri Code 最佳实践' },
        zh: {
            title: '最佳实践：我们自己是怎么用 Cancri Code 的',
            date: '2026年9月26日',
            category: '教程',
            readTime: '10 分钟阅读',
            paragraphs: [
                'Cancri Code 的大部分代码，是用 Cancri Code 写的。这不是一句口号，而是我们每天的工作方式。下面这些做法，是我们在自己身上反复踩坑之后留下来的，不一定适合所有人，但大概率能帮你少走弯路。',
                '<strong>1. 先让它读，再让它写</strong>',
                '给任务之前，先问一句"这块代码是怎么工作的"。让模型先把相关文件读一遍、讲清楚调用关系，再动手。你会发现，它后面写的代码明显更贴合项目本身的风格——因为它真的看过了，而不是在猜。',
                '<strong>2. 把"做完"定义清楚</strong>',
                '"修好这个 bug"是一个模糊的目标。"修好之后，这个测试要能通过，并且不能影响另外两个页面"是一个可以被验证的目标。你给的验收标准越具体，它交回来的东西就越接近你想要的。',
                '<strong>3. 大任务先出计划</strong>',
                '涉及多个模块的改动，先打开计划模式，让它把思路、改动范围和风险点列出来。你花两分钟看一遍计划，能省掉之后二十分钟的返工。计划被你认可之后，再放手让它执行。',
                '<strong>4. 一个任务，一条对话</strong>',
                '不要在同一条对话里连续塞五件不相干的事。上下文会越来越杂，模型的注意力也会被稀释。一件事做完，开新对话；需要并行的，交给子任务或者独立的工作树，互不干扰。',
                '<strong>5. 有副作用的事，放进云沙箱</strong>',
                '我们内部有一条不成文的规矩：凡是"跑一下看看会发生什么"的命令，都不在本机跑。云沙箱隔离、干净、用完即焚。你的开发机应该留给你自己。',
                '<strong>6. 相信证据，不相信"我改好了"</strong>',
                '任何 AI 都可能在没有验证的情况下说"已完成"。Cancri Code 被设计成会主动去跑测试、看输出、对比结果，但你仍然应该养成习惯：看它给出的验证证据，而不只是看结论。',
    ...
    ccUseCases: {
        overlay: 'Use Cases',
        media: { type: 'image', src: 'Logo/Cancri0550.png', alt: 'Cancri Code 典型应用场景' },
        zh: {
            title: '典型应用场景：什么时候该把事情交给 Cancri Code',
            date: '2026年9月26日',
            category: '场景',
            readTime: '9 分钟阅读',
            paragraphs: [
                '我们经常被问：Cancri Code 到底适合做什么？与其列一张功能表，不如讲几个我们和用户真正在用的场景。',
                '<strong>接手一个陌生的代码库</strong>',
                '新入职、接手外包项目、或者半年没碰过的老代码——最痛苦的不是改，而是看懂。让 Cancri Code 先把项目走读一遍：入口在哪、数据怎么流、哪些模块最脆弱。一个下午的熟悉期，可以缩短到一杯咖啡的时间。',
                '<strong>修那种"偶尔才出现"的 bug</strong>',
                '真正难的 bug 往往没法一眼看出来。Cancri Code 会先尝试复现，再缩小范围、加日志、跑测试，最后才动手修，并留下一个能防止它再次出现的测试。它不会因为"看起来像是这里"就直接改。',
                '<strong>重构与迁移</strong>',
                '升级框架版本、把一套旧接口换成新接口、把几十个文件里的同一种写法统一掉——这类工作量大、重复度高、又容不得出错。Cancri Code 能在多个文件之间保持一致的改法，并在每一步之后验证项目还能跑。',
                '<strong>上线前的压测与安全测试</strong>',
                '以前做压测、做渗透测试，要么在自己电脑上跑，要么专门去申请一台机器。现在，Cancri Code 可以直接在派欧云（PPIO）的弹性云沙箱里拉起一个隔离环境，部署、施压、收集结果，然后把机器收回。整个过程不碰你的本机，也不碰你的生产环境。',
                '<strong>让它替你去点浏览器</strong>',
                '有些问题只有在真实浏览器里才能发现：一个按钮点不动、一段流程在某一步卡住。选择带浏览器的云沙箱，Cancri Code 能自己打开页面、操作、截图、定位问题，而你可以在旁边实时看着它做。',
                '<strong>多个项目并行推进</strong>',
    ...
    ccEngineering: {
        overlay: 'Engineering at CancriCode',
        media: { type: 'image', src: 'Logo/CCODE-1.5.16.9.png', alt: 'Engineering at CancriCode' },
        zh: {
            title: 'Engineering at CancriCode：我们如何做一个值得信任的 AI 工程师',
            date: '2026年9月26日',
            category: '工程',
            readTime: '11 分钟阅读',
            paragraphs: [
                '做一个"能写代码的 AI"不难，难的是做一个"你敢把代码交给它的 AI"。这篇文章讲的不是我们用了什么技术，而是我们在做每一个决定时，心里放着的那几条原则。',
                '<strong>证据先于结论</strong>',
                '我们对 Cancri Code 最核心的要求只有一条：没有验证过的事，不能说"做完了"。它应该先读代码再下判断，先复现问题再修改，改完之后跑一遍给你看。这条原则让它慢了一点，但换来的是你不需要再替它检查一遍。',
                '<strong>让人始终在环里</strong>',
                '自动化不等于失控。读文件、搜索这些没有副作用的动作，它可以自己做；写文件、执行命令、访问外部网络这些有后果的动作，要经过你的确认，或者被限定在一个出了事也不会伤到你的地方。每一步都记录在案，随时可以回看。',
                '<strong>从本地虚拟机，到弹性云沙箱</strong>',
                '早期我们尝试过让 Cancri Code 在你的电脑上管理一台本地虚拟机。它能用，但体验并不好：安装重、占资源、对机器配置有要求，而且一旦出问题，影响的是你自己的电脑。',
                '所以我们做了一个决定：把"动手"的那部分搬到云上。今天，Cancri Code 的高风险操作运行在我们与派欧云（PPIO）合作的弹性云沙箱容器里——每个任务一台隔离的机器，按需启动，闲置自动暂停，用完即回收。你的电脑只负责和你对话，脏活累活都在云上完成。',
                '这个决定背后的逻辑很朴素：最安全的隔离，是物理上就不在同一台机器上。',
                '<strong>不绑架、可替换</strong>',
                '我们没有去 Fork 一个现成的编辑器，而是从开源世界里挑选经过验证的零件，自己组装。模型也是一样：Cancri Code 不押注某一家的模型，你可以按任务在不同模型之间切换。我们希望你留下来，是因为它好用，而不是因为你走不了。',
                '<strong>安全不是一个功能</strong>',
    ...
    ccAbout: {
        overlay: 'About CancriCode',
        media: { type: 'image', src: 'Logo/Cancri0526.png', alt: '关于 CancriCode' },
        zh: {
            title: '关于 CancriCode：写给每一位开发者的一封信',
            date: '2026年9月26日',
            category: '公司',
            readTime: '6 分钟阅读',
            paragraphs: [
                '你好，我是 Cancri Code 的联合创始人。',
                '我们开始做 Cancri Code 的原因很简单：我们自己受够了。受够了在聊天窗口和编辑器之间来回复制粘贴，受够了 AI 信誓旦旦地说"已修复"而实际上什么都没跑过，受够了为了试一段脚本而把自己的电脑搞得一团糟。',
                '我们想要的，是一位真正能干活的工程师。它能理解整个项目，而不是只看你粘贴进来的那几十行；它能自己动手去查、去改、去验证，而不是把半成品丢回给你；它能在一个安全的地方做危险的事，而不是在你的电脑上赌运气。',
                '这就是 Cancri Code。',
                '<strong>我们相信什么</strong>',
                '我们相信，AI 编程工具的下一步不是写得更快，而是交付得更可靠。一行没有被验证过的代码，写得再快也只是在给你制造新的工作。',
                '我们相信，开发者应该始终掌握最终决定权。AI 可以提议、可以执行，但什么该改、什么不该碰，由你说了算。',
                '我们相信，安全和隐私不应该是付费选项。你的代码是你的，它不会被拿去训练别人的模型。',
                '<strong>我们是谁</strong>',
    ...
    ccJoin: {
        overlay: 'Join CancriCode',
        media: { type: 'image', src: 'Logo/Cancri1.jpg', alt: '加入 CancriCode' },
        zh: {
            title: '加入 CancriCode：和我们一起定义 AI 工程师',
            date: '2026年9月26日',
            category: '招聘',
            readTime: '5 分钟阅读',
            paragraphs: [
                '我们在找一些人，一起做一件还没有标准答案的事情：让 AI 成为一位真正可以被信任的软件工程师。',
                '<strong>我们是一支怎样的团队</strong>',
                '团队很小，这意味着你做的每一个决定都会直接出现在用户面前。我们没有冗长的审批流程，但对质量有近乎固执的要求：没有证据不下结论，没有验证不说完成。这条要求我们对 Cancri Code 提，也对自己提。',
                '我们每天都在用自己做的产品。你写下的每一行代码，大概率会先被你自己用上。',
                '<strong>我们在找什么样的人</strong>',
                '对 AI 智能体真正感兴趣的工程师——不是停留在调用接口，而是关心它为什么会犯错、怎么让它少犯错。',
                '对桌面应用、编辑器、终端这类"开发者每天都要摸"的东西有品味的人。好工具的差别往往藏在那些看不见的细节里。',
    ...
    ccNews: {
        overlay: 'Cancri Code 3.0',
        media: { type: 'image', src: 'Logo/CCODE3.0.png', alt: 'Cancri Code 3.0' },
        zh: {
            title: '最新动态：Cancri Code 3.0，动手的部分搬上云',
            date: '2026年9月26日',
            category: '动态',
            readTime: '6 分钟阅读',
            paragraphs: [
                '今天，Cancri Code 3.0 正式发布。这是我们到目前为止改动最大的一个版本，而其中最重要的一件事，可以用一句话说完：Cancri Code 动手的部分，搬到了云上。',
                '<strong>告别本地虚拟机</strong>',
                '在之前的版本里，Cancri Code 要做压测、跑陌生脚本、做安全测试，依赖的是一台跑在你电脑上的本地虚拟机。它能工作，但它重、慢、挑机器，而且风险始终留在你自己的电脑上。',
                '从 3.0 开始，这些操作全部运行在我们与派欧云（PPIO）合作的弹性云沙箱容器中。每个任务一台隔离的机器，按需启动，闲置时自动暂停，任务结束就回收。你不需要安装任何虚拟化软件，也不需要为一台闲着的机器付费。',
                '<strong>能写代码，也能开浏览器</strong>',
                '云沙箱分成两种用途：一种用来跑代码，一种带有真实浏览器。选择后者，Cancri Code 可以自己打开网页、点击、填表、截图，把"只有在浏览器里才能发现的问题"也纳入它的工作范围。你可以在旁边实时看着它操作。',
                '<strong>更可靠的交付</strong>',
                '3.0 同时带来了一批我们在内部打磨了很久的改进：更稳的多文件编辑、更清楚的工作过程展示、更完善的计划模式与子任务协作。它们不那么显眼，但会让你在日常使用中少遇到很多"咦，怎么又这样"的时刻。',
                '<strong>下一步</strong>',
