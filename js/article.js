    ...
    news3: {
        ...
        zh: {
            ...
            paragraphs: [
                ...
                '本套件为第三方非官方产品，未获原版权方（Rockstar Games 及 Take-Two Interactive）的授权或认可，亦非《Grand Theft Auto V》的官方扩展内容。本套件仅为技术演示与娱乐用途，不构成上述任何方的官方扩展或联名产品。',
                '2. 授权与许可声明',
                '2.1 Sentience全部源代码以 MIT 许可证开源，见仓库 LICENSE。您可自由使用/修改，但必须同时遵守所用模型的原始许可。',
                '2.2 TACTFR与NexusV修改器原创部分保留所有权利，仅授权个人、非商业、单机线下使用。',
                '2.3 许可范围边界说明：Sentience 模块的独立源代码适用 MIT 许可证；但本套件作为整合了 TACTFR、修改器及特定模型权重的已编译整合包（二进制分发版本)，其整体使用权受本协议约束，仅授权个人、单机线下娱乐使用。未经开发者书面授权，严禁对本套件的整合包、预设配置文件或模型权重进行打包售卖、付费代安装等任何形式的商业变现行为。第三方组件权利以其原始许可为准，本协议不得限制。\n豁免条款：开发者允许并鼓励用户通过各大视频平台（如 Bilibili、YouTube、Twitch 等）对使用本套件的游戏过程进行免费或带有常规平台激励性质的录屏分享与直播。此种常规的内容创作分发行为不被视为本协议所禁止的"商业变现"，但创作者必须严格遵守本协议第 5 节关于"深度合成标识"的合规要求。',
                '3. 第三方组件与AI模型',
                ...
                '4.2 云端API模式（需用户主动配置）：用户在本地config.ini中填入自己的DeepSeek、OpenAI等API Key后，本套件会将NPC对话上下文直接发送至用户指定的服务商。开发者不中转、不存储、不查看用户的API Key或对话内容。API Key以明文存储在本地用户可访问的配置文件中，请勿分享配置文件或上传至公开仓库，强烈建议优先使用本地模式以避免任何数据传输和费用风险。使用云端模式时，您的IP地址、设备信息将由您的电脑直接发送给所选API服务商，受其隐私政策约束。',
                '4.3 数据最小化：仅发送必要上下文，不会故意发送真实姓名、身份证号、支付信息等敏感个人信息。用户应避免在与 Sentience 的对话中输入真实姓名、身份证号、住址、电话号码、支付信息等高度敏感个人信息；因自行输入导致的一切后果（包括但不限于信息泄露、被模型记忆、法律风险等）由用户自行承担。',
                '4.4 日志：本版本（含默认配置）不包含开发者自建的云端服务器，因此开发者不在云端收集、存储或处理您的任何数据。若本套件后续版本提供由开发者主导的云端服务，将另行发布专门的《隐私政策》并强制要求用户勾选同意后方可使用，本协议不涵盖该等未提供的服务。用户本地日志可自行删除。',
                ...
                '禁止用途：严禁利用本套件生成并在公共网络空间传播任何违反当地法律法规之内容，包括但不限于色情、暴力恐怖、仇恨言论、侵害隐私或诽谤等内容。开发者保留对于恶意滥用行为采取技术手段限制或追究法律责任的权利。',
                '6. 使用限制与禁止行为',
                '年龄限制：本套件仅供年满 18 周岁且具有完全民事行为能力的用户使用。',
                '严禁在GTA Online或任何Rockstar官方多人服务中使用本套件任何功能，由此导致的封号由您自负。出口管制合规：用户在使用云端API模式时，需自行确保其网络环境和调用行为符合所选API服务商所在地的出口管制法律法规（如美国的 EAR 规定）。因用户违规调用API导致的封号、IP拉黑或法律追责，开发者不承担任何连带责任。',
                ...
                '非官方渠道获取风险：本套件仅在开发者指定的官方渠道（如 Github、指定模组论坛等）免费发布。开发者对用户从任何第三方淘宝/闲鱼店铺、付费网盘或未授权代售群获取本套件所遭受的财产损失、木马病毒感染或数据泄露，拒绝承担任何法律或道义上的责任。',
                '责任限额：在适用法律允许的最大范围内，开发者对因本套件引起或与之相关的任何直接损害、间接损害、特殊损害、惩罚性损害或后果性损害不承担责任。若在某一司法管辖区法律不允许完全排除责任，则开发者对可归责责任的总体赔偿金额以法律允许的最大范围为限；若用户未向开发者支付任何费用，则最高赔偿责任上限为以100元人民币与法定赔偿限额中的较高者为准，但不影响法律规定不得排除或限制的责任。本条款不适用于故意或重大过失，不适用于法定不可免责责任。',
                ...
                '因版权方要求的终止：鉴于本套件依附于《Grand Theft Auto V》，若收到原版权方（Take-Two Interactive 或 Rockstar Games）的停止通知（Cease & Desist）或相关维权要求，开发者有权随时、无条件且不经提前通知地永久停止本套件的维护、下架相关文件并终止云端推理服务（若有），且无需向用户承担任何违约或赔偿责任，法律另有强制性规定的除外。',
                ...
                '本套件启动时显示的任何版本更新信息、公告或本协议的更新，一旦您点击确认或继续进入游戏界面，即视为您已收到通知并同意相关变更。如本协议中英文版本存在歧义，以中文为准。'
            ]
        },
        en: {
            ...
            paragraphs: [
                ...
                'This Suite is an unofficial third-party product, not authorized or endorsed by the original copyright holders (Rockstar Games and Take-Two Interactive), and is not an official extension of Grand Theft Auto V. This Suite is provided solely for technical demonstration and entertainment purposes, and does not constitute an official extension or co-branded product of any of the aforementioned parties.',
                '2. License Grant',
                ...
                '2.3 License Scope Boundary: Sentience module\'s independent source code is subject to MIT License; however, the Suite as an integrated package (binary distribution version) combining TACTFR, Modifier, and specific model weights, its overall usage rights are subject to this Agreement, licensed only for personal, single-player offline entertainment use. Without prior written authorization from Developer, it is strictly prohibited to engage in any form of commercial monetization such as repackaging for sale, paid installation services, etc. Exception Clause: Developer allows and encourages users to share game recording and live streaming of using this Suite on video platforms (such as Bilibili, YouTube, Twitch, etc.) for free or with regular platform incentive-based content creation. Such regular content creation and distribution is NOT considered "commercial monetization" prohibited by this Agreement, but creators must strictly comply with the "Deep Synthesis Identification" compliance requirements in Section 5 of this Agreement.',
                ...
                '4.4 Logs: This version (including default configuration) does not include Developer-operated cloud servers, so Developer does not collect, store, or process any of your data in the cloud. If subsequent versions of this Suite provide Developer-operated cloud services, a separate Privacy Policy will be published and users will be required to check and agree before use. This Agreement does not cover such services not currently provided. Local logs on your PC can be deleted by yourself.',
