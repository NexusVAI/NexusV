    ...
    >
      ...
      <div className="flex-1 relative overflow-auto min-h-screen md:col-span-2" ref={containerRef} onScroll={handleScroll}>
        ...
        <button
          ...
          className="fixed top-8 right-8 z-50 px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg shadow-lg transition-all duration-200"
        >
          {showMessage ? "隐藏留言" : "显示留言"}
        </button>
        ...
        {activeTab === "userInfo" && (
          ...
          >
            ...
            >
              <div className="w-[36rem] text-shadow-lg">
                <h3 className={`text-2xl font-semibold mb-4 ${userInfoOverLight ? 'text-black' : 'text-white'}`}>留言板</h3>
                <div className="space-y-3">
                  <div className="flex items-center space-x-3">
                    <div className={`w-12 h-12 backdrop-blur rounded-full flex items-center justify-center font-semibold ${userInfoOverLight ? 'bg-white/20 text-black' : 'bg-black/10 text-white'}`}>JD</div>
                    <div>
                      <p className={`font-medium ${userInfoOverLight ? 'text-black' : 'text-white'}`}>John Doe</p>
                      <p className={`text-sm ${userInfoOverLight ? 'text-black/70' : 'text-white/70'}`}>Software Engineer</p>
                    </div>
                  </div>
                  <div className="pt-2 space-y-2">
                    <textarea 
                      placeholder="写下你的留言..." 
                      className={`w-full h-24 p-3 rounded-lg backdrop-blur resize-none border focus:outline-none focus:border-white/30 ${userInfoOverLight ? 'bg-white/20 text-black placeholder-black/60 border-black/10 focus:border-black/30' : 'bg-black/20 text-white placeholder-white/60 border-white/10'}`}
                    />
                    <button className={`px-4 py-2 rounded-lg font-medium transition-colors ${userInfoOverLight ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-blue-500 hover:bg-blue-600 text-white'}`}>
                      提交留言
                    </button>
                  </div>
                </div>
