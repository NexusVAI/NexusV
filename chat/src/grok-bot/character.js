...
(function (g) {
  ...
  class GrokCharacter {
    constructor(svg, opts = {}) {
      ...
      this.partScale = 1;
      this.celebrateAt = -1;
      // ▼ CANCRI PATCH：progress 环对齐真实等待窗口。见 setProgressWindow。
    ...
    /**
     * ▼ CANCRI PATCH
     * 把 progress 环绑到一次真实等待上：一圈 = durationMs，且已经等过 elapsedMs。
     * 传 null 恢复上游行为（2500ms 循环演示 + 每 1500ms 熄一次）。
     * 上游那套是给静态展示用的，对齐真实时长时必须同时关掉熄灭循环，
     * 否则环会在倒计时中途整个消失再重来。
     */
    setProgressWindow(durationMs, elapsedMs = 0) {
      const ok = typeof durationMs === "number" && durationMs > 0;
      this.progressWindow = ok
        ? { durationMs, elapsedMs: Math.max(0, elapsedMs) }
        : null;
      this.fx.progressCycleMs = ok ? durationMs : null;
      if (ok && this.ovKind === "progress") {
        this.fx.overlayAt = performance.now() - this.progressWindow.elapsedMs;
      }
    }

    /** ▼ CANCRI PATCH：叠加层起点。对齐真实等待时要把已等过的时间倒扣回去。 */
    _overlayT0(now, kind) {
      if (kind === "progress" && this.progressWindow) {
        return now - this.progressWindow.elapsedMs;
      }
      return now;
    }
    ...
    _stepOverlay(now) {
      ...
      if (want !== this.ovTarget) {
        this.ovTarget = want;
        this.fx.overlayAt = this._overlayT0(now, want);
        this.ovRest = false;
        this.ovRestAt = 0;
      }
      let on = want != null;
      // ▼ CANCRI PATCH：对齐真实等待时长时不能走熄灭循环（见 setProgressWindow）
      const cycles = FX.CYCLE.has(this.state)
        && !(this.state === "progress" && this.progressWindow);
      if (want && cycles) {
