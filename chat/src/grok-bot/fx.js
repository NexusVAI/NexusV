...
(function (global) {
  ...
  class OverlayLayer {
    constructor() {
      ...
      this.pencilPath = "";
      this.bangPath = "";
      // ▼ CANCRI PATCH：progress 环的一圈时长可被宿主改写，用来对齐 agent 声明的真实等待。
      //   null = 沿用上游的 2500ms 循环演示动画。见 setProgressDuration。
      this.progressCycleMs = null;
    }
    ...
    paintProgress(ze, now, R) {
      ...
      // ▼ CANCRI PATCH：真实等待时长优先。上游把一圈写死成 CYCLE_ON.progress（2500ms）
      //   并额外除 0.85（演示动画提前满环）；对齐真实时长时不能提前满，故除数为 1。
      const span = this.progressCycleMs;
      const Lt = clamp((now - this.overlayAt) / (span || CYCLE_ON.progress), 0, 1);
      const yn = span ? Lt : clamp(Lt / 0.85, 0, 1);
